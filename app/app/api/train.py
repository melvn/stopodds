...
def train_models(df: pd.DataFrame, min_sample_size: int = 300, min_stops: int = 50) -> Dict[str, Any]:
    """Main training function - prioritizes LightGBM"""
    ...
    try:
        ...
        feature_cols_lgb = [col for col in df_lgb.columns 
                           if col not in ['stops', 'trips', 'id', 'created_at', 'user_agent_hash']]
        
        X_lgb = df_lgb[feature_cols_lgb]
        y = df_lgb['stops']
        trips = df_lgb['trips']
        
        lgb_results = modeler.fit_lightgbm_model(X_lgb, y, trips)
        results['lightgbm'] = lgb_results
        results['primary_model'] = 'lightgbm'
        
        # Save the trained model
        model_path = f"/tmp/stopodds_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Fallback to Poisson regression if LightGBM fails
    if 'lightgbm_error' in results:
        try:
